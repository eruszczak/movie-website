...
def get_title_data(const):
    ...
    if json:
        ...
        # do i need conver to numbers, yes because need to check if N/A
        # todo NEED CONVERT TYPES
        tomatoes = dict(
            tomato_meter=json['tomatoMeter'], tomato_rating=json['tomatoRating'], tomato_reviews=json['tomatoReviews'],
            tomato_fresh=json['tomatoFresh'], tomato_rotten=['tomatoRotten'], url_tomato=json['tomatoURL'],
        ...
        entry = Title(const=const, name=json['Title'], type=title_type, rate_imdb=json['imdbRating'],
                      runtime=json['Runtime'][:-4], year=json['Year'][:4], release_date=convert_to_datetime(json['Released'], 'json'),
                      votes=json['imdbVotes'], plot=json['Plot'],
                      **tomatoes
        )
        entry.save()
        # download_and_save_img(entry)
        ...
        for actor in json['Actor'].split(', '):
            actor, created = Actor.objects.get_or_create(name=actor)
            entry.director.add(actor)


# title exists and it is unique rating
def add_new_rating(user, title, rate, rate_date):
...
def update_from_csv(user):
    with open(user.imdb_ratings, 'r') as f:
        ...
        for num, row in enumerate(reader):
            ...
            add_new_rating(user=user, title=title, rate=row['You rated'], rate_date=rate_date)
...
def update_from_rss(user):  # maybe default suer will be admin
    itemlist = get_rss(imdb_id=user.imdb_id, source='ratings')
    if itemlist:
        for num, obj in enumerate(itemlist):
            ...
            if not Rating.objects.filter(user=user, title=title, rate=rate, rate_date=rate_date).exists():
                add_new_rating(user=user, title=title, rate=rate, rate_date=rate_date)
            # else:
